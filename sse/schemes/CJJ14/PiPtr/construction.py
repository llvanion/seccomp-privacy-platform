...
from schemes.CJJ14.PiPtr.structures import (
    PiPtrKey,
    PiPtrToken,
...
from toolkit.database_utils import (
    partition_identifiers_to_blocks,
...
class PiPtr(schemes.interface.inverted_index_sse.InvertedIndexSSE):
    ...
    def _Enc(self, K: PiPtrKey, database: dict) -> PiPtrEncryptedDatabase:
        ...
        A_len = (
            sum(
                math.ceil(len(database[keyword]) / self.config.param_B)
                for keyword in database
            )
            + 1
        )
        A = [None] * A_len
        index_size_in_A = math.ceil(
            math.log2(A_len) / 8
        )  # Fixed size of the index of A
        available_pos_list = random.sample(
            range(1, A_len), A_len - 1
        )  # index of A start at 1 !!

        for keyword in database:
            K1 = self.config.prf_f(K, b"\x01" + keyword)
            K2 = self.config.prf_f(K, b"\x02" + keyword)
            file_id_block_list = partition_identifiers_to_blocks(
                database[keyword],
                self.config.param_B,
            ...
            for j, file_id_block in enumerate(file_id_block_list):
                ...
                index_list_in_A.append(
                    int_to_bytes(index_in_A, output_len=index_size_in_A)
                )
            ...
            # partition indices
            index_block_list = partition_identifiers_to_blocks(
                index_list_in_A, self.config.param_b, index_size_in_A
            )
    ...
    def _Search(self, edb: PiPtrEncryptedDatabase, tk: PiPtrToken) -> PiPtrResult:
        ...
        for index_in_A in index_list:
            file_id_block_cipher = A[
                int_from_bytes(index_in_A)
            ]  # need to convert bytes to int
            file_id_block = self.config.ske.Decrypt(K2, file_id_block_cipher)
            result.extend(
                parse_identifiers_from_block_given_identifier_size(
                    file_id_block, self.config.param_identifier_size
                )
            )
            pass
    ...
    def EDBSetup(self, key: PiPtrKey, database: dict) -> PiPtrEncryptedDatabase:
        return self._Enc(key, database)

    def TokenGen(self, key: PiPtrKey, keyword: bytes) -> PiPtrToken:
        return self._Trap(key, keyword)

    def Search(self, edb: PiPtrEncryptedDatabase, token: PiPtrToken) -> PiPtrResult:
