...
def export_bridge_records(*,
    ...
    try:
        ...
        print(f">>> Exported {len(selected_rows)} bridge rows to {out}.")
    except Exception as e:
        print(f">>> Export bridge records error: {e}")
...
def _write_export_rows(rows: list[dict], out: Path, out_format: str, *, join_key_field: str, value_field: str):
