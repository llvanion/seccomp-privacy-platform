...
- `scripts/metadata_db.py` now provides a psycopg2 driver layer: `connect_db(dsn=…)` selects PostgreSQL when `dsn` is non-empty; `adapt_sql`, `placeholder`, `is_postgres`, and `connect_db_with_retry` helpers normalize parameter syntax across backends. `scripts/init_metadata_db.py` accepts `--db-dsn`. Other scripts still use SQLite by default; remaining call sites need `adapt_sql` wrapping to complete the F1 PostgreSQL migration.
...
- PostgreSQL driver layer (F1-a): `scripts/metadata_db.py` — `connect_db(dsn=…)`, `is_postgres`, `placeholder`, `adapt_sql`, `connect_db_with_retry`; `scripts/init_metadata_db.py --db-dsn`
