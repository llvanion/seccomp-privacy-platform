...
- `scripts/metadata_db.py` now provides a psycopg2 driver layer: `connect_db(dsn=…)` selects PostgreSQL when `dsn` is non-empty; `adapt_sql`, `placeholder`, `is_postgres`, `row_to_dict`, and `connect_db_with_retry` helpers normalize rows and parameter syntax across backends. `scripts/init_metadata_db.py`, `scripts/import_run_metadata.py`, `scripts/query_metadata.py`, `scripts/manage_metadata_db.py`, `scripts/serve_metadata_api.py`, `scripts/resolve_api_identity.py`, `scripts/check_metadata_schema_portability.py`, and `scripts/benchmark_read_adapters.py` accept `--db-dsn`; query/audit/platform-health adapters accept `--metadata-db-dsn` for identity-resolution paths. SQLite remains the default local path; F1-b still needs a live PostgreSQL gate.
