...
- `scripts/render_operator_console_manifest.py` now emits `operator_console_manifest_report/v1` for Track-E3 (operator console product baseline): it reads `config/operator_console/console_manifest.json` (`console_manifest/v1`, 8 sections — `home` / `jobs` / `audit` / `catalog` / `permissions` / `recovery` / `observability` / `compliance` — covering 25 endpoints across 5 platform_roles), validates against `schemas/console_manifest.schema.json`, and asserts the static placeholder `config/operator_console/index.html` references the manifest at runtime. Default contract smoke renders the report and asserts `status=ok`, all 8 sections present, `static_index_references_manifest=true`, and `commerce_ops_owner` / `compliance_auditor` / `recovery_service_operator` all appear in the role set.
...
- Operator console product baseline (Track-E3): `scripts/render_operator_console_manifest.py`, `schemas/console_manifest.schema.json`, `schemas/operator_console_manifest_report.schema.json`, `config/operator_console/console_manifest.json`, `config/operator_console/index.html`; plan: `docs/OPERATOR_CONSOLE_PRODUCT_PLAN.md`
