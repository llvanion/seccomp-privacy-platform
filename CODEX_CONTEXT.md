...
- `scripts/benchmark_bridge.py` now emits `bridge_benchmark/v1` for G3 bridge prepare-job scale testing: it generates deterministic synthetic e-commerce JSONL inputs, runs the existing Rust bridge `prepare-job` command in production mode, and records output row counts, audit decision, job-meta counts, throughput, and peak RSS. `scripts/benchmark_smoke.py --target bridge-scale --scale <n>` invokes it. Local 2026-05-08 release-binary runs passed at 100k server / 100k client rows (0.366s, RSS 44,932 KB) and 1M server / 1M client rows (4.437s, RSS 422,864 KB). CPU top-hotspot evidence remains an operator profiling follow-up because local `perf` is blocked by `perf_event_paranoid=4` and `cargo flamegraph` is unavailable.
