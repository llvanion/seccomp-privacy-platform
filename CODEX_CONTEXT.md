...
- Record recovery service now performs HMAC-SHA256 request-body signing: the client attaches `request_id`, `request_timestamp_utc`, and `request_signature = HMAC-SHA256(token, "{request_id}:{ts}:{op}")` to every `recover` payload; HTTP transport also sends `X-Request-Signature` and `X-Request-Signature-Algorithm` headers; the server verifies the signature (constant-time) when auth_token is set and the client sends one; `request_signature_verified` and `signature_algorithm` are recorded in `sse_record_recovery_service_audit/v1` and frozen as stable properties.
...
- Pipeline SLO benchmark coverage now also has `scripts/benchmark_pipeline_slo.py`, which generates deterministic 10k/10k/1k-overlap fixtures, derives expected intersection metrics, runs the existing file-handoff pipeline when the SSE/PJC runtime is available, reads `pipeline_observability/v1` stage durations, and emits `pipeline_slo_benchmark/v1`; default contract smoke validates a fixture-only report while live 10k timing remains an explicit operator/runtime check.
- PJC-only benchmark coverage now also has `scripts/benchmark_pjc.py`, which runs `a-psi/moduleA_psi/scripts/run_pjc.sh` against the checked-in `bridge/out/sse_demo_job/` fixture and emits `pjc_benchmark/v1` after validating `intersection_size=2` and `intersection_sum=425`; it also has `generated_scale_csv` for deterministic PJC scale fixtures with per-mode `scale` metadata and optional `peak_rss_kb`, while measured 100k/1M PJC timing remains an operator-runtime follow-up.
