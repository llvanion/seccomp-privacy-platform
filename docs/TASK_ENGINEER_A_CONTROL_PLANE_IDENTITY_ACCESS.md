...
2. **`A2` 已完成（2026-05-05）**：OpenFGA tuple sync + check adapter
   ...
   - **2026-05-06 补强**：`scripts/openfga_http.py`、`config/openfga.example.json`、`schemas/openfga_config.schema.json` 已把 sync/check adapter 扩展到 live OpenFGA HTTP backend；默认仍是 SQLite fallback。`check_json_contracts.sh` 新增可选 live smoke：只有设置 `OPENFGA_ENDPOINT` + `OPENFGA_STORE_ID` 时才 apply tuples、执行 live check、并通过 `authority_governance_report/v1` 汇总。
   - **2026-05-06 E2-a 收口**：`config/openfga_authorization_model.json` 与 `scripts/setup_openfga_model.py` 固定了 OpenFGA store/model setup 入口；默认 smoke 跑 dry-run，`--execute` 才访问 live OpenFGA。
...
6. **`A6` 已完成（2026-05-05）**：远端 authority smoke + runbook 收口
   ...
   - 2026-05-06 后，OpenFGA live check 也可通过 `OPENFGA_ENDPOINT` + `OPENFGA_STORE_ID` 显式启用；未设置时默认 CI 不依赖真实 OpenFGA。
   - 2026-05-06 后，Keycloak client-credentials、Vault AppRole/PKI、AWS KMS adapter 也已收成 repo-side 工具：`request_oidc_client_credentials.py`、`issue_mtls_certs.py`、`cloud_kms_adapter.py`。默认均为 dry-run/mock，不把真实 authority source 变成主链路依赖。
