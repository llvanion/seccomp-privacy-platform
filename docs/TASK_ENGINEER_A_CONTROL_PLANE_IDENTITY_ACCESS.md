...
1. `A1`：issuer-backed identity proxy baseline
   - 把当前 bearer-token / issuer-subject resolver 接到更正式的前置认证入口
   - 固定 `issuer -> caller / tenant_id / service_id / roles` 注入规则
2. `A2`：OpenFGA tuple sync + check adapter
   - 以 `authz_tuple_export/v1` 为同步基线
   - 先覆盖 metadata/query/audit/platform-health 读侧与 submit/execute 入口
3. `A3`：真实 Vault / cloud KMS adapter baseline
   - 把 `vault_kv` / `vault_http` 从本地兼容 fixture 推到远端 authority source
4. `A4`：service identity + token lifecycle
   - 收紧 recovery service / key agent / external KMS 的服务身份、轮换、撤销与 drift
5. `A5`：policy / key / identity mutation governance 联动
   - 把 policy drift、key drift、issuer rotation 收成统一 operator 视图
6. `A6`：远端 authority smoke + runbook 收口
