...
| # | Risk | Fix | Files |
| --- | --- | --- | --- |
| PJC_MTLS task 6 | "Add contract smoke coverage for bucket suppression and DP metadata" was the last item we had no permanent guard for. | New `scripts/check_bucket_dp_smoke.py` builds a two-bucket attribution fixture (one above k, one below), runs `policy_release.py` and `policy_postprocess_buckets.py`, and asserts six invariants: (a) below-k bucket is suppressed with nulls, (b) above-k bucket is released with `dp_noise_applied=true`, (c) `public_report.json` surfaces `dp_noise_applied=true` and positive `dp_epsilon`, (d) `policy_release.py --require-dp` exits non-zero without knobs, (e) same for `policy_postprocess_buckets.py --require-dp`, (f) `released_sum == max(0, round(raw_sum + dp_noise))`. Wired into default `scripts/check_json_contracts.sh` and into the `check_ci_smoke.sh` `py_compile` list. | `scripts/check_bucket_dp_smoke.py`, `scripts/check_json_contracts.sh`, `scripts/check_ci_smoke.sh` |
...
| # | Risk | Fix | Files |
| --- | --- | --- | --- |
| S5 | `public_report.json` shipped `input_sizes`, `rate_limit_used/max`, `bridge`, `details`, and (via `policy_postprocess_buckets.py`) a `debug.per_bucket_results` block — direct frame-count and per-bucket leak to anyone with read access to the public release. | Two opt-in flags. `policy_release.py --public-report-redact-operator-fields` strips the five operator-only keys from `public_report.json` (sets `operator_fields_redacted=true`) and routes the full set into a sibling `operator_release_report/v1` document (path is `--operator-report-path` or `<out>.operator.json`). `policy_postprocess_buckets.py --public-report-redact-operator-fields` skips writing `debug.per_bucket_results` / `debug.bucket_policy` into the public report, leaving only a redaction marker; the dedicated `bucket_public_report.json` is unaffected and the operator console still has the full data there. `run_bucketed_scale_test.sh` passes both flags by default. | `a-psi/moduleA_psi/scripts/policy_release.py`, `policy_postprocess_buckets.py`, `run_bucketed_scale_test.sh`, `scripts/check_bucket_dp_smoke.py` (smoke extended) |
...
- **PJC_MTLS task 6**: `python3 scripts/check_bucket_dp_smoke.py --out-dir tmp/bucket_dp_smoke` returns `status=ok` with all six invariants satisfied; below-k bucket suppressed, above-k bucket released with `dp_noise_applied=true`, `released_sum == max(0, round(raw_sum + dp_noise))` confirmed numerically. Round 7 extended it to eight invariants — the redacted run also shows `public_report_operator_fields_redacted=true` and `operator_report_carries_full_set=true`.
