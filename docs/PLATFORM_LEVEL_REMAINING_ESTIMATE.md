...
| 任务 | 剩余 block | 约合工时 | 说明 |
| --- | ---: | ---: | --- |
| owner：隐私内核与接口治理 | 6 | 30h | ~~Block3~~ ✓, ~~Block4~~ ✓, Block1 部分 ✓；还差 record recovery 独立边界完整 authn/lifecycle、handoff 收紧 |
...
1. 串行视角：`36 blocks = 180h`（owner 已完成 Block3 + Block4 + Block1-partial）
