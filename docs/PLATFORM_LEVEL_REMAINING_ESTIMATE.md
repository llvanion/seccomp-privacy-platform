...
| G — Scale & optimization | 0 | G1/G2-a/G2-b/G3/G4-a/G4-b/G5/G6/G7/G8 全部本地完成（G4-a 100k 222s、G4-b 三轮 back-to-back 稳定、G5 10k pipeline 34.9s 均在 2026-05-09 收口；1M unary ceiling 已于 2026-05-14 通过 chunked streaming rerun 解除，仍需 S4 生产 worker/resource gate） |
...
24. ~~G4-b：connection reuse + memory ceiling~~ ✓ 三轮 back-to-back 10k PJC 生命周期稳定、RSS 36-39 MB 无 leak；scaling 表 1k→10k→100k 落入 `pjc_benchmark/v1`；1M unary live ceiling 于 2026-05-12 复跑为 32.72 min / 2.25 GB peak RSS / `exit_code=1`（grpc 512MB cap），随后 2026-05-14 chunked streaming rerun 通过：34:05.18 / 2.20 GB peak RSS / `exit_code=0` / `intersection_size=200000` / `intersection_sum=20020100000`。
...
- **G4-b (memory ceiling + connection reuse)**：`scripts/benchmark_pjc.py --mode generated_scale_csv --server-items 10000 --client-items 10000 --overlap 0.2 --iterations 3` 跑了三轮 back-to-back PJC server+client 生命周期，每轮 `intersection_size=2000`、`intersection_sum=2,201,000`、exit_code=0，per-iter dur 47.0s/28.8s/36.6s（首轮 cold-start，后两轮 OS cache 暖），peak RSS 36-39 MB 稳定无 leak；100k peak RSS 260.8 MB 表明 server 持单倍候选集而非双缓冲。1M×1M unary measurement 于 2026-05-12 复跑时触发 `GRPC_MAX_MESSAGE_MB=512` 上限；2026-05-14 新增 chunked bidirectional streaming 后，`PJC_GRPC_STREAM_CHUNK_ELEMENTS=4096` 的 1M×1M rerun 通过：34:05.18 wall time、peak RSS 2,204,740 KB ≈ **2.20 GB**、`exit_code=0`、`intersection_size=200000`、`intersection_sum=20020100000`。`run_pjc.sh` 每轮 spawn / teardown server，"connection reuse" 在当前 PJC binary 下指 runner 可重入、log/work dir 不冲突；真正的 production worker、cancel/retry、resource preflight 和超 1M sharding 归 `PRODUCTION_SECURITY_COMPLETION_PLAN.md` 的 S4。
