...
Local measured runs against the bazel-built PJC server/client (`a-psi/private-join-and-compute/bazel-bin/private_join_and_compute/{server,client}`); 1k/10k/100k and 10k×3 were measured on 2026-05-09, and the 1M row was rerun on 2026-05-12:
...
| **1M / 1M (×0.2)** | **32.72 min (1,963s)** | n/a (failed) | **2.25 GB** (2,248,648 KB) | n/a (`exit_code=1`) | n/a (`exit_code=1`) | 1 |

100k×100k passes the G4-a SLO of < 300s with ~26% headroom. 10k×3 back-to-back proves runner re-entrancy + no per-iteration RSS growth. The 1M×1M row was rerun on 2026-05-12 and reached the bazel-built PJC binary's unary gRPC message-size ceiling (`GRPC_MAX_MESSAGE_MB=512`) on the single-machine reference run; `tmp/pjc_benchmark_1m.json` records `generated_at_utc=2026-05-12T14:59:41.194852Z`, `duration_ms=1963428.32`, `peak_rss_kb=2248648`, `exit_code=1`, and null intersection metrics. On 2026-05-13, the PJC client/server gained a compatible chunked `HandleStream` transport and the local smoke forced `PJC_GRPC_STREAM_CHUNK_ELEMENTS=1` while still returning `intersection_size=2` and `intersection_sum=425`. On 2026-05-14, the streaming 1M×1M rerun passed with `PJC_GRPC_STREAM_CHUNK_ELEMENTS=4096`: `tmp/pjc_streaming_1m_benchmark.json` records `duration_ms=2045196.066`, wall time **34:05.18**, `peak_rss_kb=2204740`, `exit_code=0`, `intersection_size=200000`, and `intersection_sum=20020100000`. This validates that the large repeated encrypted sets no longer have to fit into one gRPC protobuf message; memory and CPU remain the next scale constraints.

S4 preflight gate (2026-05-14): before launching the PJC server/client pair, run `scripts/preflight_pjc_job.py --resource-limits config/pjc_resource_limits.example.json` to estimate input rows / bytes / peak memory / streaming frame count and reject jobs that would breach the matched scope's limits. The preflight report (`pjc_preflight/v1`) and the `transport_mode` / `frame_count` / `resource_limits` / `preflight_decision` fields newly accepted by `pjc_audit/v1` let benchmark runs document which limits admitted them. Use `bash scripts/verify_pjc_preflight_gate.sh` to re-validate the gate semantics without spinning up PJC; see [docs/OPS_RUNBOOK.md §S4](/home/llvanion/Desktop/seccomp-privacy-platform/docs/OPS_RUNBOOK.md) for the full table of decision codes. For new scale rows, propose a scope under `auto_demo` (or the relevant caller) with `max_input_rows`, `max_input_bytes`, `max_estimated_memory_mb`, and `max_frame_count` matched to the target benchmark before kicking off the run.
